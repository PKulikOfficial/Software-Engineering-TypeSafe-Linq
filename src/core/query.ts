...
export interface Query<T,R> {
    ...
    Include: <K extends keyof Pick<T, {[K in keyof T]: T[K] extends Array<object> ? K : never}[keyof T]>, T2 extends T[K] extends Array<infer I> ? I : never, K2 extends keyof T2>
        (prop: K, query: (q: Table<T2>) => Query<T2,Pick<T2,K2>>) =>
        Query<T,R & {[key in K]: [Pick<T2,K2>]}>
}
...
export const Query = <T,R>(Data: Array<T>, Result: Array<R> ) : Query<T,R> => ({
    ...
    Include: function<K extends keyof Pick<T, {[K in keyof T]: T[K] extends Array<object> ? K : never}[keyof T]>, T2 extends T[K] extends Array<infer I> ? I : never, K2 extends keyof T2>
        (prop: K, query: (q: Table<T2>) => Query<T2,Pick<T2,K2>>): Query<T,R & {[key in K]: [Pick<T2,K2>]}> {
...
export interface LazyQuery<T,R> {
    ...
    Include: <K extends keyof Pick<T, {[K in keyof T]: T[K] extends Array<object> ? K : never}[keyof T]>, T2 extends T[K] extends Array<infer I> ? I : never, K2 extends keyof T2>
        (prop: K, query: (q: Table<T2>) => Query<T2,Pick<T2,K2>>) =>
        LazyQuery<T,R & {[key in K]: [Pick<T2,K2>]}>
}
